...
class TwbBundleFormRadioTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testRenderOptionsWithPrependingLabel() {
        ...
        $this->assertEquals(
                '<label>test<input value="0" checked></label>', $oReflectionMethod->invoke($this->formRadioHelper, new \Laminas\Form\Element\Radio(), array('test'), array('test'), array())
        );
    }
    ...
    public function testRenderOptionsWithDefineAttributesId() {
        ...
        $this->formRadioHelper->setLabelPosition(\TwbBundle\Form\View\Helper\TwbBundleFormRadio::LABEL_PREPEND);
        $this->assertEquals(
                '<label>test1<input id="test_id" value="0" checked></label></div><div class="radio"><label>test2<input value="1"></label>', $oReflectionMethod->invoke($this->formRadioHelper, new \Laminas\Form\Element\Radio(), array('test1', 'test2'), array('test'), array('id' => 'test_id'))
        );
    }
